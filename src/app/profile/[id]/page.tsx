  ...
  return (
    <div className="container mx-auto p-4 text-white bg-gradient-to-b from-[#2e026d] to-[#15162c] min-h-screen">
      <div className="profile-tabs flex gap-4 mb-6">
        <Link 
          href={`/profile/${userId}`} 
          className="profile-tab cursor-pointer font-semibold border-b-2 border-purple-500 text-xl"
        >
          Профиль
        </Link>
        <Link 
          href={`/portfolio/${userId}`} 
          className="profile-tab cursor-pointer font-semibold text-xl hover:text-purple-400 transition-colors"
        >
          Портфолио
        </Link>
        {isCurrentUser && (
            ...
            <div className="space-y-6">
              <div>
                ...
                <div className="space-y-2 text-gray-300">
                  ...
                  {user.contactPhone && <p>Телефон: {user.contactPhone}</p>}
              ...
              <div>
                ...
                {user.biography ? (
                  <p className="text-gray-300 whitespace-pre-line">{user.biography}</p>
                ) : (
                  <p className="text-gray-500 italic">- Тут пока ничего нет -</p>
                )}
              </div>
                ...
                <div>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-gray-300">
                    <div>
                      ...
                      <p>{user.birthday ? new Date(user.birthday).toLocaleDateString('ru-RU') : 'Не указана'}</p>
                    </div>
                    <div>
                      <p className="font-medium">Пол:</p>
                      <p>{user.sex === 'woman' ? 'Женский' : 'Мужской'}</p>
                    </div>
                  </div>
                </div>
              )}
                ...
                >
                  ...
                  {item.link && (
                    <a
                      ...
                      target="_blank"
                      rel="noopener noreferrer"
                      className="inline-block text-blue-400 hover:text-blue-300 hover:underline text-sm"
                    >
                      Ссылка на проект →
                    </a>
                  )}
